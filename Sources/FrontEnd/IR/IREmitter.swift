...
internal struct IREmitter {
  ...
  private mutating func _emitDeinitialize(
    ...
    guard let recordFields = program.withTyper(typing: module, { (tp) in tp.fields(of: t) }) else {
      // todo handle enums and other non-record types.
    }
    ...
    // Deinitialize each stored part individually.
    return recordFields.indices.allSatisfy { (i) in
      _emitDeinitialize(_subfield(s, at: [i]))
    }
  }
